...
class DittoTalkingHeadService(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, StartFrame):
            ...
            if not self._initialized or self._is_interrupting:
                ...
                # Push audio immediately - don't buffer
                # Let the transport handle audio playback timing
    ...
    async def _consume_and_push_video(self):
        """Consume video frames from queue and push immediately.

        Pushes video frames immediately as they're generated by Ditto.
        Audio is pushed separately when it arrives from TTS.

        Video frames include PTS timestamps for transport-level synchronization,
        but no artificial pacing delays are applied for low latency.
        """
        logger.info(f"{self}: Video playback task started (immediate push, no delays)")
            ...
            while True:
                try:
                    ...
                    if frames_pushed == 1:
                        ...
                        logger.info(f"{self}: Pushed {frames_pushed} video frames")
                    ...
                    # Push video frame immediately
                    # Audio is pushed separately as it arrives from TTS
                    await self.push_frame(output_frame)

                    logger.debug(f"{self}: Pushed video frame {frames_pushed} (PTS: {audio_timestamp:.3f}s)")
