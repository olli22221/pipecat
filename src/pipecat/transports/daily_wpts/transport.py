...
"""Daily transport implementation for Pipecat with PTS support.
...
**PTS Support:**
This version of the Daily transport includes PTS (Presentation Timestamp) support
for both audio and video frames. When frames have a `pts` attribute, they will be
paced using asyncio.sleep() to maintain proper audio-video synchronization. This
ensures frames are written to Daily at the correct wall-clock time relative to
their timestamps, providing better A/V sync for talking head and video generation
use cases.

Both audio and video frames are independently paced based on their PTS timestamps,
ensuring perfect synchronization even when frames are generated at different times.

This is particularly useful when using services like DittoTalkingHeadService that
generate timestamped audio and video frames synchronized together.
"""
...
class DailyTransportClient(EventHandler):
    ...
    def __init__(
        ...
        # PTS-based video frame pacing for proper audio-video synchronization
        self._video_pts_base_time = None  # Wall-clock time when first frame with PTS is written
        self._video_pts_base = None  # PTS value of first frame
        self._video_frame_write_lock = asyncio.Lock()  # Lock to serialize frame writes with pacing

        # PTS-based audio frame pacing for proper audio-video synchronization
        self._audio_pts_base_time = None  # Wall-clock time when first audio frame with PTS is written
        self._audio_pts_base = None  # PTS value of first audio frame
        self._audio_frame_write_lock = asyncio.Lock()  # Lock to serialize audio writes with pacing
    ...
    async def write_audio_frame(self, frame: OutputAudioRawFrame) -> bool:
        """Write an audio frame to the appropriate audio track with PTS-based pacing.

        If the frame has a PTS (Presentation Timestamp) attribute, this method
        will pace the frame delivery to maintain proper audio-video synchronization.
        Frames are delayed using asyncio.sleep() to ensure they are written at the
        correct wall-clock time relative to their PTS timestamps.
        ...
        if audio_source:
            # Use lock to serialize audio writes when using PTS-based pacing
            async with self._audio_frame_write_lock:
                # Check if frame has PTS timestamp for pacing
                if hasattr(frame, 'pts') and frame.pts is not None:
                    current_time = asyncio.get_event_loop().time()

                    # Initialize base time on first audio frame with PTS
                    if self._audio_pts_base_time is None:
                        self._audio_pts_base_time = current_time
                        self._audio_pts_base = frame.pts
                        logger.info(f"{self}: Audio PTS-based pacing initialized (base_time: {self._audio_pts_base_time:.3f}s, base_pts: {self._audio_pts_base:.3f}s)")

                    # Calculate when this audio frame should be written based on its PTS
                    pts_offset = frame.pts - self._audio_pts_base
                    target_write_time = self._audio_pts_base_time + pts_offset
                    wait_time = target_write_time - current_time

                    # Wait if we're early
                    if wait_time > 0:
                        logger.debug(f"{self}: Waiting {wait_time*1000:.1f}ms before writing audio (PTS: {frame.pts:.3f}s)")
                        await asyncio.sleep(wait_time)
                    elif wait_time < -0.1:  # More than 100ms late
                        logger.warning(f"{self}: Audio frame is {-wait_time*1000:.1f}ms late (PTS: {frame.pts:.3f}s, processing can't keep up)")

                logger.debug(f"{self}: Writing audio frame (size: {len(frame.audio)} bytes)")
                future = self._get_event_loop().create_future()
    ...
    async def write_video_frame(self, frame: OutputImageRawFrame) -> bool:
        """Write a video frame to the camera device with PTS-based pacing.
        ...
        if not frame.transport_destination and self._camera:
            # Use lock to serialize frame writes when using PTS-based pacing
            async with self._video_frame_write_lock:
                # Check if frame has PTS timestamp for pacing
                if hasattr(frame, 'pts') and frame.pts is not None:
                    current_time = asyncio.get_event_loop().time()

                    # Initialize base time on first frame with PTS
                    if self._video_pts_base_time is None:
                        self._video_pts_base_time = current_time
                        self._video_pts_base = frame.pts
                        logger.info(f"{self}: PTS-based pacing initialized (base_time: {self._video_pts_base_time:.3f}s, base_pts: {self._video_pts_base:.3f}s)")

                    # Calculate when this frame should be written based on its PTS
                    pts_offset = frame.pts - self._video_pts_base
                    target_write_time = self._video_pts_base_time + pts_offset
                    wait_time = target_write_time - current_time

                    # Wait if we're early
                    if wait_time > 0:
                        logger.debug(f"{self}: Waiting {wait_time*1000:.1f}ms before writing frame (PTS: {frame.pts:.3f}s)")
                        await asyncio.sleep(wait_time)
                    elif wait_time < -0.1:  # More than 100ms late
                        logger.warning(f"{self}: Frame is {-wait_time*1000:.1f}ms late (PTS: {frame.pts:.3f}s, processing can't keep up)")
    ...
    async def start(self, frame: StartFrame):
        ...
        if self._params.video_out_enabled and not self._camera:
            ...
            logger.info(f"{self}: Camera device created successfully")
